...
TABLE_REGEX = re.compile(r"^(table|list of tables|list of figures)", re.IGNORECASE)
...
async def add_bookmarks_to_pdf_file(
    input_pdf_path, output_pdf_path, include_list_of_tables=False
):
    log = []
    ...
    if toc_entries:
        ...
        for title, page in toc_entries:
            # Add option to include list of tables/figures
            if include_list_of_tables or not TABLE_REGEX.match(title):
    ...
    )  # Save the bookmarked PDF to the specified output path
