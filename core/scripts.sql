...
-- Drop and create usercompany table
...
-- Drop and create pdfMasterConfig table
...
CREATE TABLE IF NOT EXISTS pdffile (
    ...
    status varchar(50) DEFAULT 'pending', -- Changed DEFAULT FALSE to 'pending' as status is varchar
...
-- Drop and create pdfUserConfig table
...
-- Drop and create sitestatus table
...
CREATE TABLE IF NOT EXISTS sitestatus (
    ...
    createdOn TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    updatedOn TIMESTAMP WITH TIME ZONE NULL,
    CONSTRAINT fk_pdf_file_sitestatus FOREIGN KEY (pdf_file_id) REFERENCES pdffile(id) -- Corrected FK name
);
